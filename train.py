...
def train_loop():
    ...
    history.append(GameMove(go_board.create_zero_state(0), 
    ...
    for iter_index in np.arange(NUM_ITER):
        ...
        for game_index, start_move in enumerate(starting_states):
            logging.info("Game: %d: start player: %d, start state: \n%s", game_index, start_move.state.player, go_board.to_pretty_print(start_move.state.pos[-1]))
        ...
        new_model_win_ratio = np.sum(outcomes == OUTCOME_WIN_PLAYER_1)/len(outcomes)
